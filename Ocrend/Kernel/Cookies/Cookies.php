...
class Cookies {
    ...
    public function reviveSessions() {
        ...
        if($config['sessions']['user_cookie']['enable']) {
            # Obtener
            $user_session = $session->get($this->get('session_hash_website') . '__user_id_');
            $salt = $this->get('appsalt_website');
            $encrypt = $this->get('appencrypt_website');
            ...
            if(null == $user_session && null != $salt && null != $encrypt && Helper\Strings::chash($salt ?? '_', $encrypt ?? '/$')) {
                ...
                $this->set('session_hash_website', md5(time()) , $config['sessions']['user_cookie']['lifetime']);
                ...
                $session->set($this->get('session_hash_website') . '__user_id_',(int) $id_user);
            }
        }       
    }
}
