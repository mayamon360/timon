...
abstract class Controllers {
    ...
    */
    protected function __construct(IRouter $router, $configController = []) {
        ...
        $this->is_logged = null != $session->get($cookie->get('session_hash_website') . '__user_id_');
        ...
        $this->template->addGlobal('logotipo', (new Model\Datos)->logotipos());
        $this->template->addGlobal('datos', (new Model\Datos)->datosGenerales());
        $this->template->addGlobal('redes_sociales', (new Model\Datos)->redesSociales());
        $this->template->addGlobal('colores', (new Model\Datos)->coloresPlantilla());
        $this->template->addGlobal('categorias', (new Model\Datos)->categorias());
        $this->template->addGlobal('subcategorias', (new Model\Datos)->subcategorias());
        $this->template->addGlobal('libroRand', (new Model\Libro)->libroRand());
        $this->template->addGlobal('cantidad_carrito', (new Model\Compra)->cantidadCarrito());

        # Datos del usuario actual
        if ($this->is_logged) {
          $this->user = (new Model\Usuarios)->getOwnerUser();
        ...
        $this->template->addGlobal('cabeceras', (new Model\Datos)->cabeceras($this->method));
    ...
    private function knowVisitorPermissions() {
      ...
      if ($this->controllerConfig['users_not_logged'] && $this->is_logged) {
        Helper\Functions::redir($config['build']['url'] . 'cuenta/mis-datos');
      }
    }
