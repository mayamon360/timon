...
abstract class Controllers {
    ...
    */
    ...
    /**
      * Arreglo con la información de la plantilla.
      *
      * @var array 
    */
    protected $plantilla = array();
    ...
    */
    protected function __construct(IRouter $router, $configController = []) {
        ...
        if ($this->is_logged) {
            ...
            $this->template->addGlobal('menu', (new Model\Menu)->getMenuUser());
        }

        # Icono que aparece al lado de la URL
        $this->plantilla = (new Model\Comercio)->datosPlantilla();
        $this->template->addGlobal('iconoURL', $this->plantilla["icono"]);
        ...
        $this->knowVisitorPermissions($router->getController());
     ...
     * @return void
     */
    private function knowVisitorPermissions($controlador) {
        ...
        if($this->controllerConfig['module_access']){
            $module_access = (new Model\Menu)->module_access($this->user, $controlador);
            if(!$module_access){
                Helper\Functions::redir();
            }  
        }
